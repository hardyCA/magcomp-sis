-- Clientes en ventas: registrar_venta ahora acepta p_cliente (nombre).
-- Si el nombre no existe, crea el cliente; si existe, lo reutiliza.
-- Guarda cliente_id en la venta.

drop function if exists public.registrar_venta(jsonb, text, numeric);
...
begin
  ...
  v_cliente_nombre := nullif(trim(coalesce(p_cliente, '')), '');
  if v_cliente_nombre is not null then
    select id into v_cliente_id from public.clientes where nombre = v_cliente_nombre;
    if not found then
      insert into public.clientes (nombre)
      values (v_cliente_nombre)
  ...
  insert into public.ventas
    (numero, usuario_id, moneda, tipo_cambio, subtotal, descuento, total, cliente_id)
  values
    (v_numero, auth.uid(), p_moneda, v_tasa, v_subtotal, p_descuento, v_total, v_cliente_id)
...
revoke all on function public.registrar_venta(jsonb, text, text, numeric) from public;
grant execute on function public.registrar_venta(jsonb, text, text, numeric) to authenticated;